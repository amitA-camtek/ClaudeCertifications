# W11 — Integration & Hands-On Exercises

**Domain:** Exam Prep · All Domains
**Budget:** 2 days × 2.5 h = 5 h

## Study Day (2.5 h)

| Time | Block | Task |
|---|---|---|
| 0:00–0:10 | Warmup | Scan `notes/weak_spots.md` from W01–W10 |
| 0:10–1:20 | Exercise | **Exercise 1 — Multi-Tool Agent with Escalation Logic** |
| 1:20–2:20 | Exercise | **Exercise 2 — Claude Code Team Workflow Configuration** |
| 2:20–2:30 | Summary | Notes on integration pain points → `notes/study_day.md` |

## Practice Day (2.5 h)

| Time | Block | Task |
|---|---|---|
| 0:00–0:50 | Exercise | **Exercise 3 — Structured Data Extraction Pipeline** |
| 0:50–1:40 | Exercise | **Exercise 4 — Multi-Agent Research Pipeline** |
| 1:40–2:20 | Exam | Start **Full Practice Exam 1** (50 Q, all 6 scenarios) — aim to finish; carry over if needed |
| 2:20–2:30 | Weak spots | Identify 2 weakest domains → `notes/weak_spots.md` (feeds W12) |

**Deliverables:** Exercises 1–4 in `exercises/`, Practice Exam 1 score + wrong-answer review in `practice_test/full_exam_1.md`, 2 weakest domains identified.
